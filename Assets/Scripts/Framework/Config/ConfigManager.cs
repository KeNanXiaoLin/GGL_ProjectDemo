...
namespace KNXL.AutoGenerated.Data
{
    ...
    public class ConfigManager
    {
        ...
        public void Init()
        {
            InitSyncFromManifest();
        }

        /// <summary>
        /// 同步初始化配置（从清单文件加载所有表）
        /// </summary>
        public void InitSyncFromManifest()
        {
            if (IsInitialized)
            {
            ...
            {
                LoadTableFromMeta(tableMeta);
            }

        }

        private void LoadTableFromMeta(TableMetaInfo tableMeta)
        {
            try
            {
                ...
                var table = LoadTable(configType);
                _configTables[table.TableName] = table;
        ...
        /// </summary>
        /// <returns>协程</returns>
        public IEnumerator InitFromManifestCoroutine()
        {
            if (IsInitialized)
            {
                ConfigHelper.LogInfo("ConfigManager", "已初始化，无需重复执行");
            ...
            foreach (var tableMeta in _configManifest.tables)
            {
                yield return MonoMgr.Instance.StartCoroutine(LoadTableFromMetaCoroutine(tableMeta));
        ...
        /// <returns>协程</returns>
        private IEnumerator LoadTableFromMetaCoroutine(TableMetaInfo tableMeta)
        {
        ...
        public ConfigTable<T> LoadTable<T>() where T : BaseConfig, new()
        {
            // 获取泛型类型对应的表名（移除Cfg前缀）
        ...
        /// <returns>配置表实例（IConfigTable接口）</returns>
        public IConfigTable LoadTable(Type configType)
        {
            // 检查类型是否继承BaseConfig
            if (!typeof(BaseConfig).IsAssignableFrom(configType))
            {
                ConfigHelper.LogError("ConfigManager", $"类型{configType.Name}未继承BaseConfig");
                return null;
            }
            ...
            // 使用反射创建泛型ConfigTable<T>实例
            try
            {
                // 创建泛型ConfigTable类型
                Type genericTableType = typeof(ConfigTable<>).MakeGenericType(configType);

                // 创建表实例
                var table = Activator.CreateInstance(genericTableType, tableName, "id") as IConfigTable;
            ...
            {
                ConfigHelper.LogError("ConfigManager", $"创建表实例失败：{e.Message}");
                return null;
            }
        }
